...
(function ($) {
    ...
    var methods = {
        init: function (options) {
            ...
            function ajax(action) {
                ...
                var $editbutton = $table.find("tr:last").find('button.tabledit-edit-button');
                var temprow = $editbutton.hasClass('temprow');
                if (temprow) {
                    action = "add";
                }
                if (!serialize) {
                    ...
                    toolbar: function () {
                        if (settings.editButton || settings.deleteButton) {
                            ...
                            if (settings.deleteButton) {
                                ...
                                confirmButton = '<button type="button" class="tabledit-confirm-button ' + settings.buttons.confirm.class + '" style="display: none;">' + confirmButtonHtml + '</button>';
                            }

                            // Create save button.
                            if (settings.editButton && settings.saveButton) {
                                saveButton = '<button type="button" class="tabledit-save-button ' + settings.buttons.save.class + '" style="display: none;">' + saveButtonHtml + '</button>';
                            }

                            // Create restore button.
                            if (settings.deleteButton && settings.restoreButton) {
                                restoreButton = '<button type="button" class="tabledit-restore-button ' + settings.buttons.restore.class + '" style="display: none;">' + restoreButtonHtml + '</button>';
                            }

                            var toolbar = '<div class="tabledit-toolbar ' + settings.toolbarClass + '" style="text-align: left;flex-wrap: nowrap;"><div class="' + settings.groupClass + '" style="float: none;">' 
                                                                                     + editButton 
                                                                                     + deleteButton 
                                                                                     + saveButton
                                                                                     + confirmButton
                                                                                     + restoreButton + '</div></div>';

                            // Add toolbar column cells for normal cell
                            $table.find('tbody > tr:not(".' + settings.noEditClass + '")').append('<td style="width: 190px;">' + toolbar + '</td>');
                            // Add toolbar column with a ban on row editing
                            $table.find('tbody > tr.' + settings.noEditClass).append('<td></td>');
                        }
                    }
                }
            };
            ...
            function buildRowId() {
                var $id = $table.find("tr").find("td.rowid");
            ...
            buildRowId();
            // Add new row
            var $addrow = $("#addnewrow");
            $addrow.click(function(event) {
                if (event.handled !== true) {
                    // Insert new row to end of table
                    // Stop, the default action of the event will not be triggered
                    var $tr = $table.find("tr:last");
                    var $editbutton = $tr.find('button.tabledit-edit-button');
                    var activated = $editbutton.hasClass('active');
                    var temprow = $editbutton.hasClass('temprow');
                    var disabled = $editbutton[0].getAttribute("disabled");
                    if (activated) {
                        $tr.find('button.tabledit-edit-button').trigger( "click" );
                    }
                    if (temprow) {
                        $tr.find('button.tabledit-edit-button').trigger( "click" );
                        buildRowId();
                        ...
                        var $tr = $table.find("tr:last")
                        if (disabled) {
                            $tr.removeClass("tabledit-deleted-row");
                            $tr.removeClass("text-muted");
                            var $disabledtoolbar = $tr.find("button")
                            $disabledtoolbar.each(function(){
                                $(this)[0].removeAttribute("disabled");
                            });

                            $tr.find("button.warning").hide();
                            
                        }
                        var $td = $tr.find("td");
                        $td.each(function() {
                            $td.find(".tabledit-input").text("");
                            $td.find(".tabledit-input").val("");
                        });
                        $tr.find('button.tabledit-edit-button').addClass('temprow');
                        $tr.find('button.tabledit-edit-button').trigger( "click" );
                        buildRowId();
                        event.handled = true;
                    }
    ...
    $.fn.Tabledit.defaults = {
        ...
        buttons: {
            edit: {
                class: 'button btn-sm btn-default',
                html: '<span class="mif-pencil mif-lg"></span>',
                action: 'edit'
            },
            delete: {
                class: 'button btn-sm btn-default',
                html: '<span class="mif-cross mif-lg"></span>',
                action: 'delete'
            },
            save: {
